#include "constant_texture_test.h"
#include "texture/constant_texture.h"
#include <texture/texture_test.h>
#include <test_assert_helper.h>
#include <cute.h>
#include <vector>
#include <tuple>

namespace texture
{
	using namespace ray_tracing_core::texture;
	using namespace ray_tracing_core;

	void constant_texture_channels_test(void)
	{
		std::vector<std::tuple<core::Color, math::AlphaValue, bool, bool, ConstantTexture>> test_data
		{
			{core::Color(0), 1.0, false, false, ConstantTexture(core::Color(0), 1.0)},
			{core::Color(0), 0.5, false, true, ConstantTexture(core::Color(0), 0.5)}
		};

		for (auto [expected_color, ecpected_alpha_value, expected_depends_on_texture, expected_has_alpha, texture] : test_data)
		{
			auto [actual_color, actual_alpha] = texture.channels(core::TextureCoordinate::null(), math::Point3D(0));
			auto actual_depends_on_texture = texture.depends_on_texture_coordinates();
			auto actual_has_alpha = texture.has_alpha_channel();
			assert_equal_color(expected_color, actual_color, 0.001);
			ASSERT_EQUAL_DELTA(ecpected_alpha_value, actual_alpha, 0.001);
			ASSERT_EQUAL(expected_depends_on_texture, actual_depends_on_texture);
			ASSERT_EQUAL(expected_has_alpha, actual_has_alpha);
		}
	}
}
