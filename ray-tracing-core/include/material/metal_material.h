...
namespace ray_tracing_core::material
{
    ...
    bool MetalMaterial::scatter(const math::Ray &ray, core::HitRecord &hit_record, core::ScatterRecord &scatter_record) const
    {
        ...
        if (glm::dot(scattered_ray.direction, nv) > 0.0)
        {
            ...
            scatter_record.probability_density_function = pdf::ProbabilityDensityFunctionType::CosinePDFFromNormal;
    ...
    core::Color MetalMaterial::emit(const math::Ray &ray_in, const core::HitRecord &hit_record) const
    {
        return core::Color(0);
    }
