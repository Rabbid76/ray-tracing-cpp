...
namespace ray_tracing_core_unit_test::material
{
    ...
    void metal_material_hit_test()
    {
        ...
        {
            auto actual_hit = MetalMaterial(0.5, &albedo)
                    .hit(core::HitRecord::empty());
            TEST_ASSERT_EQUAL(expected_hit, actual_hit);
        }
    }

    void metal_material_scatter_test()
    {
        ...
        for (auto [color, alpha_value, ray_in, hit_point] : test_data)
        {
            ...
            MetalMaterial material(0.5, &albedo);
            ...
            TEST_ASSERT_EQUAL(pdf::ProbabilityDensityFunctionType::CosinePDFFromNormal, scatter_record.probability_density_function);
            TEST_ASSERT_EQUAL(&material, scatter_record.material);
        }
    }

    void metal_material_scattering_pfd_test()
    {
        ...
        for (auto [expected_distance, normal, direction] : test_data)
        {
            ...
            auto actual_distance = MetalMaterial(0.5, &albedo)
                    .scattering_pdf(
    ...
    void metal_material_emit_test()
    {
        auto albedo = texture::ConstantTexture(core::Color(0), 0.0f);
        auto actual_emit = MetalMaterial(0.5, &albedo)
                .emit(
                        math::Ray::new_ray(math::Point3D(0), math::Vector3D(0)),
    ...
    void metal_material_has_texture_test()
    {
        ...
        {
            auto actual_has_texture = MetalMaterial(0.5, albedo.get())
                    .has_texture();
            TEST_ASSERT_EQUAL(expected_has_texture, actual_has_texture);
        }
    }

    void metal_material_has_mask_test()
    {
        std::vector<std::tuple<bool, texture::ConstantTexture>> test_data
                {
        ...
        {
            auto actual_has_mask = MetalMaterial(0.5, &albedo)
                    .has_mask();
