...
namespace ray_tracing_core_unit_test::material
{
    ...
    void isotropic_material_hit_test()
    {
        ...
        {
            auto actual_hit = IsotropicMaterial(&albedo)
                    .hit(core::HitRecord::empty());
            TEST_ASSERT_EQUAL(expected_hit, actual_hit);
        }
    }

    void isotropic_material_scatter_test()
    {
        ...
        for (auto [color, alpha_value, ray_in, hit_point] : test_data)
        {
            ...
            IsotropicMaterial material(&albedo);
    ...
    void isotropic_material_scattering_pfd_test()
    {
        ...
        for (auto [expected_distance, normal, direction] : test_data)
        {
            ...
            auto actual_distance = IsotropicMaterial(&albedo)
                    .scattering_pdf(
    ...
    void isotropic_material_emit_test()
    {
        auto albedo = texture::ConstantTexture(core::Color(0), 0.0f);
        auto actual_emit = IsotropicMaterial(&albedo)
                .emit(
                        math::Ray::new_ray(math::Point3D(0), math::Vector3D(0)),
    ...
    void isotropic_material_has_texture_test()
    {
        ...
        {
            auto actual_has_texture = IsotropicMaterial(albedo.get())
                    .has_texture();
            TEST_ASSERT_EQUAL(expected_has_texture, actual_has_texture);
        }
    }

    void isotropic_material_has_mask_test()
    {
        std::vector<std::tuple<bool, texture::ConstantTexture>> test_data
                {
        ...
        {
            auto actual_has_mask = IsotropicMaterial(&albedo)
                    .has_mask();
