...
std::vector<uint8_t> Renderer::get_rgba8() const
{
    ...
    for (uint32_t i = 0; i < cx * cy; ++i)
    {
        ...
        pixel_data[i * 4] = static_cast<uint8_t>(std::lround(std::sqrt(color[0]) * 255.0));
        pixel_data[i * 4 + 1] = static_cast<uint8_t>(std::lround(std::sqrt(color[1]) * 255.0));
        pixel_data[i * 4 + 2] = static_cast<uint8_t>(std::lround(std::sqrt(color[2]) * 255.0));
